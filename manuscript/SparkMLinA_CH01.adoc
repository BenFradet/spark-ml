...
Another important concept is an _Estimator_. An Estimator is something that
learns a machine learning model from a DataFrame. It effectively produces a
Transformer that will, in turn, add a column containing the predictions
...
Calling `fit()` on a Pipeline will result in a pass that goes through each stage
and calls the `fit()` method on it if it's an Estimator.

This step produces a _PipelineModel_, a Transformer since it's the result of
calling `fit()` on an Estimator, which contains only Transformers since the
Estimators in the original Pipeline have been converted to Transformers.

Calling `transform()` on a PipelineModel is going to trigger a call to every
...
Pipeline of the top row. You can see that the Estimators have been converted to
Transformers: StringIndexer became _StringIndexerModel_ and
DecisionTreeClassifier became _DecisionTreeClassificationModel_.

.Calling `transform()` on the PipelineModel produced by calling `fit()` on the Pipeline
image::../images/pipeline_transform.png[]

The top row in figure 1.4 represents the PipelineModel produced when calling the
`fit()` method on the Pipeline.
